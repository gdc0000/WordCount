"""Legacy Streamlit launcher and package compatibility shim.

This file exists so `streamlit run wordcount.py` keeps working, while regular
imports resolve to the package in `src/wordcount/` instead of this legacy
module shadowing it.
"""
...
# Make this module behave like a package so `import wordcount.core` resolves to
# the real implementation under `src/wordcount`.
__path__ = [str(Path(__file__).resolve().parent / "src" / "wordcount")]
...
def main() -> None:
    ...
    def _wordlists_key(uploaded_wordlists, prefixes):
        return tuple(
            sorted(
                (uploaded_file.name, prefixes.get(uploaded_file.name, ""))
